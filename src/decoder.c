...
static pk_decoder_token_t *new_token(
    pk_decoder_t *self,
  ...
  if (previous) {
    ++previous->ref_count;
    token->cost = previous->cost + arc->weight + acoustic_cost;
  } else {
...
static void delete_token(pk_decoder_t *self, pk_decoder_token_t *token) {
